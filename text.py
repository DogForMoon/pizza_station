...
about_text = """Как ты уже понял это школьный проект. Над проектом работали Лёша Алексеев (дизайн, техническая информация) и Игорь Михелев (разработчик), а также руководитель проекта Игорь Александрович.
Если ты заметил какие-то ошибки в работе бота или у тебя есть предложения пиши сюда -> @wilfii"""

technical_info = """Немного занудства от разработчика ~_~
1. Заказать пиццу можно только в день выдачи.
2. Удалить заказ можно максимум за урок до выдачи пиццы (за 40 минут, если быть точнее). Поэтому если в меню удаления заказов нету того, который ты хочешь удалить, значит время вышло.
3. Заказать пиццу также можно только на следующую перемену (50 минут, чтобы было время отменить заказ)
