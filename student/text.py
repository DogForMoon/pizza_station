...
welcome_text = """Привет! Это чат-бот для заказа продуктов из школьного буфета.
Пиши /order и оформляй заказ!
...
pizza_text = "Я гляжу, ты решил что-то выбрать! Смотри, какие категории у меня\
 есть:"

choice_product = "Теперь выбери какой ты хочешь продукт"
...
not_time = "Сегодня уже нельзя сделать заказ. Возвращайся завтра"
...
1. Заказать наименование можно только в день выдачи.
2. Удалить заказ можно максимум за урок до выдачи (за 40 минут, если\
 быть точнее). Поэтому если в меню удаления заказов нету того, который\
 ты хочешь удалить, значит время вышло.
3. Заказать наименование также можно только на следующую перемену (50 минут,\
 чтобы было время отменить заказ).
4. Ты можешь выбрать за раз несколько товаров, но только разных. К сожалению,\
 два одинаковых продукта заказать не получиться.
